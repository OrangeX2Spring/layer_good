...
def parse_args():
    ...
    parser.add_argument("--obj-id", type=int, help="Omit to search all objects")
    parser.add_argument("--min-visib-fract", type=float, default=0.98)
    parser.add_argument("--max-visib-fract", type=float, default=1.0,
                        help="Lower this to hunt for occluded instances instead")
    ...
    for scene_dir in sorted(split_dir.iterdir()):
        ...
        for image_key, annotations in gt.items():
            for gt_id, annotation in enumerate(annotations):
                if args.obj_id is not None and annotation["obj_id"] != args.obj_id:
                    continue
                info = gt_info[image_key][gt_id]
                if not args.min_visib_fract <= info["visib_fract"] <= args.max_visib_fract:
                    continue
                if info["px_count_visib"] < args.min_visible_pixels:
                    continue
                candidates.append((
                    -info["px_count_visib"], int(scene_dir.name), int(image_key), gt_id,
                    info["visib_fract"], annotation["obj_id"],
                ))

    if not candidates:
        raise SystemExit("No view met the visibility filters")

    print(f"{'obj':>4} {'scene':>6} {'image':>6} {'gt':>3} {'visib':>6} {'px':>8}")
    for neg_px, scene_id, image_id, gt_id, visib, obj_id in sorted(candidates)[: args.top_k]:
        print(f"{obj_id:4d} {scene_id:6d} {image_id:6d} {gt_id:3d} {visib:6.3f} {-neg_px:8d}")
