...
def main(args):
    ...
    runs = args.work / 'runs' / args.scene
    runs.mkdir(parents=True)
    archive = args.datasets / f'rgbd_dataset_{args.scene}.zip'
    ...
    input_tar = args.out / f'{args.tag}_inputs_{args.scene}.tar'
    archive_inputs(inputs, input_tar)
    release_page_cache(inputs, input_tar)
    base = dict(scene=args.scene, scene_dir=str(inputs), resize_dim=308, interval=50,
                cap=20, layer='encoder', max_gt_difference=.02,
                evaluate_trajectory=False)
    write_json(args.work / 'protocol.json', dict(base=base, stage=args.stage,
    ...
    def run_one(name, policy, frames, fraction=.5, evaluate=False):
        ...
        print('RUN', name, frames, flush=True)
        started = time.time()
    ...
    print('FULL RETENTION FIDELITY OK', flush=True)
    ...
    with np.load(full / 'final_scene.npz') as scene:
        assert scene['frame_ids'].tolist() == retained
        assert np.isfinite(scene['poses']).all()
    write_json(args.work / 'GATES_OK.json', dict(full_retention_max_error=float(np.max(np.abs(a-b))),
        prefix_frames=1100, frames=length, evictions=sum(len(e['evicted']) for e in events),
        passed=True))
    for result in runs.iterdir():
        destination = args.out / f'{args.tag}_{args.scene}_{result.name}.tar'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--scene', choices=LONG_SCENES, required=True)
